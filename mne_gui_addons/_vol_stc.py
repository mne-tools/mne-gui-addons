...
def _check_consistent(items, name):
    ...
    for item in items[1:]:
        if item != items[0]:
            raise RuntimeError(
                f"Inconsistent attribute {name}, " f"got {items[0]} and {item}"
            )
    return items[0]
...
def _get_src_lut(src):
    ...
    rr = np.concatenate(
        [s["rr"][this_inuse.astype(bool)] for s, this_inuse in zip(src[offset:], inuse)]
    )
    shape = _check_consistent([this_src["shape"] for this_src in src], "src['shape']")
    ...
    for this_inuse in inuse:
        ...
        lut[this_inuse] = np.arange(n_vertices_seen, n_vertices_seen + n_vertices)
        n_vertices_seen += n_vertices
    lut = np.reshape(lut, shape, order="F")
    src_affine_ras = _check_consistent(
        [this_src["mri_ras_t"]["trans"] for this_src in src], "src['mri_ras_t']"
    )
    src_affine_src = _check_consistent(
        [this_src["src_mri_t"]["trans"] for this_src in src], "src['src_mri_t']"
    )
...
class VolSourceEstimateViewer(SliceBrowser):
    ...
    def __init__(
        ...
        if src is None:
            raise NotImplementedError(
                "`src` is required, surface source "
        ...
        if not isinstance(data, np.ndarray) or data.ndim != 5:
            raise ValueError(
                "`data` must be an array of dimensions "
                "(n_epochs, n_sources, n_ori, n_freqs, n_times)"
            )
        if group:
            if not isinstance(inst, (list, tuple)) and len(inst) != data.shape[0]:
                raise ValueError(
                    "Group-level results (group=True) number of "
                    "`inst`s does not match `data`, expected "
                    f"a list of {data.shape[0]} `inst`s, "
                    f"got a {type(inst)} of length {len(inst)}"
                )
        else:
            if isinstance(inst, (BaseEpochs, EpochsTFR)) and data.shape[0] != len(inst):
                raise ValueError(
        ...
        if (
            hasattr(inst, "freqs")
            and not group
            and not (np.iscomplexobj(data) or data.dtype == COMPLEX_DTYPE)
        ):
            raise ValueError(
                "Complex data is required for time-frequency " "source estimates"
            )
        ...
        self._is_complex = (
            np.iscomplexobj(self._data) or self._data.dtype == COMPLEX_DTYPE
        )
        self._baseline = "none"
        ...
        self._pos_support = (
            self._is_complex or self._data.shape[2] > 1 or (self._data >= 0).all()
        )
        ...
        if self._data.shape[2] > 1 and not self._is_complex:
            ...
            self._vector_mapper, self._vector_data = self._renderer.quiver3d(
                *self._src_rr.T,
                *(VECTOR_SCALAR * self._stc_vectors_masked.T),
                color=None,
                mode="2darrow",
                scale_mode="vector",
                scale=1,
            ...
            self._vector_actor.GetProperty().SetLineWidth(2.0)
        ...
        if any([this_src["type"] == "vol" for this_src in self._src]):
            ...
            origin = apply_trans(
                self._mri_scan_ras_t, self._src_vox_mri_t[:3, 3] - spacing / 2.0
            )
    ...
    def _get_min_max_val(self):
        ...
        return [
            self._cmap_sliders[i].value() / SLIDER_WIDTH * self._stc_range
            + self._stc_min
            for i in (0, 2)
        ]
    ...
    def _update_stc_pick(self):
        ...
        self._stc_data_vol = self._apply_baseline_correction(self._stc_data_vol)
    ...
    def _update_stc_volume(self):
        ...
        self._stc_img = _threshold_array(self._stc_img, *self._get_min_max_val())
    ...
    def _pick_epoch(self, stc_data):
        ...
        if self._epoch_idx == "Average":
            ...
            if stc_data.dtype == COMPLEX_DTYPE:
                stc_data = np.sum(
                    _int_complex_conj(stc_data) // stc_data.shape[0],
                    axis=0,
            ...
            if stc_data.dtype == COMPLEX_DTYPE:
                stc_data = stc_data["re"].astype(np.complex64) + 1j * stc_data["im"]
    ...
    def _apply_baseline_correction(self, stc_data):
        ...
        if self._baseline != "none":  # do baseline correction
            stc_data = rescale(
                stc_data.astype(float),
                times=self._inst.times,
    ...
    def _pick_stc_vertex(self, stc_data):
        ...
        if (
            all(
                [
                    coord >= 0 and coord < dim
                    for coord, dim in zip(src_coord, self._src_lut.shape)
                ]
            )
            and self._src_lut[src_coord] >= 0
        ):
    ...
    def _configure_toolbar(self):
        ...
        if self._data.shape[0] > 1:
            ...
            self._epoch_selector.setSizeAdjustPolicy(QComboBox.AdjustToContents)
    ...
    def _configure_sliders(self):
        ...
        self._time_slider = make_slider(
            0, self._inst.times.size - 1, self._t_idx, self._update_time
        )
        ...
        self._alpha_slider = make_slider(
            0, SLIDER_WIDTH, int(self._alpha * SLIDER_WIDTH), self._update_alpha
        )
    ...
    def _configure_status_bar(self, hbox=None):
        ...
        self._baseline_selector.addItems(
            ["none", "mean", "ratio", "logratio", "percent", "zscore", "zlogratio"]
        )
        self._baseline_selector.setCurrentText("none")
        self._baseline_selector.currentTextChanged.connect(self._update_baseline)
    ...
    def _configure_data_plot(self):
        ...
        canvas, self._fig = _make_mpl_plot(
            dpi=96, tight=False, hide_axes=False, invert=False, facecolor="white"
        )
        self._fig.axes[0].set_position([0.12, 0.25, 0.73, 0.7])
        self._fig.axes[0].set_xlabel("Time (s)")
        min_idx = np.argmin(abs(self._inst.times))
        self._fig.axes[0].set_xticks([0, min_idx, self._inst.times.size - 1])
        self._fig.axes[0].set_xticklabels(self._inst.times[[0, min_idx, -1]].round(2))
        stc_data = self._pick_stc_image()
        if self._f_idx is None:
            self._fig.axes[0].set_facecolor("black")
            self._stc_plot = self._fig.axes[0].plot(stc_data[0], color="white")[0]
            self._stc_vline = self._fig.axes[0].axvline(x=self._t_idx, color="lime")
            self._fig.axes[0].set_ylabel("Activation (AU)")
            self._cax = None
        else:
            self._stc_plot = self._fig.axes[0].imshow(
                stc_data, aspect="auto", cmap=self._cmap, interpolation="bicubic"
            )
        ...
        self._fig.canvas.mpl_connect("button_release_event", self._on_data_plot_click)
    ...
    def set_baseline(self, baseline=None, mode=None):
        ...
        self._update = False
        self._baseline_selector.setCurrentText("none" if mode is None else mode)
        if baseline is not None:
            baseline = _check_baseline(
                baseline, times=self._inst.times, sfreq=self._inst.info["sfreq"]
            )
            tmin, tmax = baseline
    ...
    def _update_baseline(self, name):
        ...
        self._cmap = _get_cmap("hot" if name == "none" and self._pos_support else "mne")
        self._go_to_extreme_button.setText(
            "Go to Max" if name == "none" and self._pos_support else "Go to Extreme"
        )
    ...
    def _update_baseline_tmin(self, event):
        ...
        tmin = self._inst.times[
            np.clip(  # find nearest time
                self._inst.time_as_index(tmin, use_rounding=True)[0],
                0,
                self._inst.times.size - 1,
            )
        ]
    ...
    def _update_freq(self, event=None):
        ...
        self._freq_label.setText(f"Freq = {self._inst.freqs[self._f_idx].round(2)} Hz")
        if self._update:
    ...
    def _update_time(self, event=None):
        ...
        self._time_label.setText(f"Time = {self._inst.times[self._t_idx].round(2)} s")
        if self._update:
            self._update_stc_images()  # just need volume updated here
    ...
    def _update_cmap(
        self, event=None, draw=True, update_slice_plots=True, update_3d=True
    ):
        """Update the colormap."""
        if not self._update:
            return
        ...
        if self._f_idx is None:
            self._fig.axes[0].set_ylim([self._stc_min, self._stc_min + self._stc_range])
        else:
    ...
    def go_to_extreme(self):
        ...
        stc_idx, f_idx, t_idx = np.unravel_index(
            np.nanargmax(abs(self._stc_data_vol)), self._stc_data_vol.shape
        )
        if self._f_idx is not None:
    ...
    @fill_doc
    def set_3d_view(
        self, roll=None, distance=None, azimuth=None, elevation=None, focalpoint=None
    ):
