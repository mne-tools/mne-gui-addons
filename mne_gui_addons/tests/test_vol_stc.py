...
def _fake_stc(src_type="vol"):
    ...
    ) + 1j * rng.integers(-1000, 1000, size=(n_epochs, nuse, 3, freqs.size, times.size))
...
def test_stc_viewer_io(renderer_interactive_pyvistaqt):
    ...
    with pytest.raises(
        NotImplementedError,
        match="surface source estimate " "viewing is not yet supported",
    ):
    ...
    with pytest.raises(ValueError, match="`data` must be an array"):
        VolSourceEstimateViewer(
            "foo", subject="sample", subjects_dir=subjects_dir, src=src, inst=epochs_tfr
        )
    with pytest.raises(ValueError, match="Number of epochs in `inst` does not match"):
        VolSourceEstimateViewer(stc_data[1:], src=src, inst=epochs_tfr)
    with pytest.raises(RuntimeError, match="ource vertices in `data` do not match "):
        VolSourceEstimateViewer(
            stc_data[:, :1],
            subject="sample",
            subjects_dir=subjects_dir,
    ...
    with pytest.raises(ValueError, match="Frequencies in `inst` do not match"):
        VolSourceEstimateViewer(stc_data[:, :, :, 1:], src=src, inst=epochs_tfr)
    ...
    with pytest.raises(ValueError, match="Times in `inst` do not match"):
        VolSourceEstimateViewer(stc_data[:, :, :, :, 1:], src=src, inst=epochs_tfr)
...
def test_stc_viewer_display(renderer_interactive_pyvistaqt):
    ...
    plot_data = np.linalg.norm((stc_data[0] * stc_data[0].conj()).real, axis=1)[stc_idx]
    assert_allclose(plot_data, viewer._stc_plot.get_array())

    # test clicking on stc plot
    _fake_click(viewer._fig, viewer._fig.axes[0], (0, 0), xform="data", kind="release")
    ...
    epochs = mne.EpochsArray(
        epochs_tfr.data[:, :, 0].real, epochs_tfr.info, tmin=epochs_tfr.tmin
    )
    ...
    with pytest.raises(
        ValueError, match="Source estimate does " "not contain frequencies"
    ):
        viewer.set_freq(10)
    ...
    assert_allclose(
        np.linalg.norm(stc_time_data[0], axis=1)[stc_idx][0],
